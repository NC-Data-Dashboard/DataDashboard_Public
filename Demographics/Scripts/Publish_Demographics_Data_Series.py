...
backup_df = pd.read_csv('./Updates/STG_WNCD_Demographics_FRED_Data_Series.txt', sep='\t')
...
df1 = pd.read_csv('./Updates/STG_FRED_Civilian_Labor_Force_by_County_Persons.txt', sep='\t')
...
df1 = df1.melt(id_vars=['GeoArea_FIPS', 'GeoArea_Name', 'Economic_Measure_Code', 'Economic_Measure_Name', 'Unit_of_Measure_Code', 'Calculation_Type', 'Default_Scale'], var_name='Date', value_name='Estimated_Value')
...
df2 = df2.melt(id_vars=['GeoArea_FIPS', 'GeoArea_Name', 'Economic_Measure_Code', 'Economic_Measure_Name', 'Unit_of_Measure_Code', 'Calculation_Type', 'Default_Scale'], var_name='Date', value_name='Estimated_Value')

df2['Estimation_Qualifier'] = ''


# Education Rate
df3 = pd.read_csv('./Updates/STG_FRED_People_25_Years_and_Over_Who_Have_Completed_an_Associates_Degree_or_Higher_5year_estimate_by_County_Percent.txt', sep='\t')
df3['Economic_Measure_Code'] = 'FRED_S1501ACSTOTAL0_00000'
df3['Economic_Measure_Name'] = 'People 25 Years and OVer Who Have Completed an Associates Degree or Higher (5-year estimate) by County (Percent)'
...
df3 = df3.melt(id_vars=['GeoArea_FIPS', 'GeoArea_Name', 'Economic_Measure_Code', 'Economic_Measure_Name',  'Unit_of_Measure_Code', 'Calculation_Type', 'Default_Scale'], var_name='Date', value_name='Estimated_Value')

df3['Estimation_Qualifier'] = ''


# Population
df4 = pd.read_csv('./Updates/STG_FRED_Resident_Population_by_County_Thousands_of_Persons.txt', sep='\t')
...
df4 = df4.melt(id_vars=['GeoArea_FIPS', 'GeoArea_Name', 'Economic_Measure_Code', 'Economic_Measure_Name',  'Unit_of_Measure_Code', 'Calculation_Type', 'Default_Scale'], var_name='Date', value_name='Estimated_Value')
...
columns = ['GeoArea_FIPS', 'GeoArea_Name', 'PZ_Name', 'WDB_Name','Economic_Measure_Code', 'Economic_Measure_Name', 'Data_Period_Business_Key','Estimated_Value', 'Estimation_Qualifier', 'Unit_of_Measure_Code', 'Default_Scale', 'Data_Period_Type','Data_Period_Name','Data_Period_Begin_Datetime', 'Calculation_Type']
df = df[columns]
...
c.execute("""sp_rename 'STG_WNCD_Demographics_FRED_Data_Series', 'STG_WNCD_Demographics_FRED_Data_Series_BACKUP';""")
...
df.to_sql(
    "STG_WNCD_Demographics_FRED_Data_Series", con=engine, if_exists="replace", index=False
)
