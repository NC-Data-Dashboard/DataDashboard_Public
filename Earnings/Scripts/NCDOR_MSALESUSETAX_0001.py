...
print("Check line 14, 33, 34, 39, and 46!")
...
df = df.drop(
    columns=["County.1", "Collections*.1", "and Purchases*", "and Purchases*.1"]
)
...
# Get overall data
df_overall = pd.read_csv("./Updates/STG_BEA_MSALESUSETAX_0001.txt", sep="\t")

# Append new data to overall data
df_end = df_overall.append(df_main)

# Save new data source
df_end.to_csv("./Updates/STG_BEA_MSALESUSETAX_0001.txt", sep="\t")
