...
df8 = pd.read_csv('./Updates/STG_BEA_CA6N_Supplements_to_Wages_and_Salaries.txt', sep='\t')
...
df9 = pd.read_csv('./Updates/STG_BEA_CA6N_Employer_Contributions_for_Employee_Pension_and_Insurance_Funds.txt', sep='\t')
...
df10 = pd.read_csv('./Updates/STG_BEA_CA6N_Employer_Contributions_for_Government_Social_Insurance.txt', sep='\t')
...
# Mining
df15 = pd.read_csv('./Updates/STG_BEA_CA6N_Mining_Quarrying_and_Oil_and_Gas_Extraction.txt', sep='\t')
...
# Transportation
df21 = pd.read_csv('./Updates/STG_BEA_CA6N_Transportation_and_Warehousing.txt', sep='\t')
...
# Real Estate
df24 = pd.read_csv('./Updates/STG_BEA_CA6N_Real_Estate_and_Rental_and_Leasing.txt', sep='\t')
...
df25 = pd.read_csv('./Updates/STG_BEA_CA6N_Professional_Scientific_and_Technical_Services.txt', sep='\t')
...
df26 = pd.read_csv('./Updates/STG_BEA_CA6N_Management_of_Companies_and_Enterprises.txt', sep='\t')
...
# Admin Support
df27 = pd.read_csv('./Updates/STG_BEA_CA6N_Administrative_and_Support_and_Waste_Management_and_Remediation_Services.txt', sep='\t')
...
# Health Care
df29 = pd.read_csv('./Updates/STG_BEA_CA6N_Health_Care_and_Social_Assistance.txt', sep='\t')
...
# Arts Entertainment
df30 = pd.read_csv('./Updates/STG_BEA_CA6N_Arts_Entertainment_and_Recreation.txt', sep='\t')
...
# Accommodation
df31 = pd.read_csv('./Updates/STG_BEA_CA6N_Accommodation_and_Food_Services.txt', sep='\t')
...
# Government
df33 = pd.read_csv('./Updates/STG_BEA_CA6N_Government_and_Government_Enterprises.txt', sep='\t')
...
df_list = [df2, df3, df4, df5, df6, df7, df8, df8, df9, df10, df11, df12, df13, df14, df15, df16, df17, df18, df19, df20, df21, df22, df23, df24, df25, df26, df27, df28, df29, df30, df31, df32, df33]
...
df34 = pd.read_csv('./Updates/STG_FRED_Unemployment_Rate_by_County_Percent.txt', sep='\t')
...
df34 = df34.rename(columns = {'Region Name':'GeoArea_Name', 'Region Code':'GeoArea_FIPS'})

df34 = df34.melt(id_vars=['GeoArea_FIPS', 'GeoArea_Name', 'Economic_Measure_Code', 'Economic_Measure_Name', 'Published_UOM', 'Unit_of_Measure_Code', 'Calculation_Type', 'Default_Scale', 'Estimation_Qualifier'], var_name='Date', value_name='Published_Value')

df34['Estimated_Real_Value'] = df34['Published_Value']


# Civilian Labor Force
df35 = pd.read_csv('./Updates/STG_FRED_Civilian_Labor_Force_by_County_Persons.txt', sep='\t')
...
df35 = df35.rename(columns = {'Region Name':'GeoArea_Name', 'Region Code':'GeoArea_FIPS'})

df35 = df35.melt(id_vars=['GeoArea_FIPS', 'GeoArea_Name', 'Economic_Measure_Code', 'Economic_Measure_Name', 'Published_UOM', 'Unit_of_Measure_Code', 'Calculation_Type', 'Default_Scale', 'Estimation_Qualifier'], var_name='Date', value_name='Published_Value')

df35['Estimated_Real_Value'] = df35['Published_Value']


# Resident Population
df36 = pd.read_csv('./Updates/STG_FRED_Resident_Population_by_County_Thousands_of_Persons.txt', sep='\t')
...
df36 = df36.rename(columns = {'Region Name':'GeoArea_Name', 'Region Code':'GeoArea_FIPS'})

df36 = df36.melt(id_vars=['GeoArea_FIPS', 'GeoArea_Name', 'Economic_Measure_Code', 'Economic_Measure_Name', 'Published_UOM', 'Unit_of_Measure_Code', 'Calculation_Type', 'Default_Scale', 'Estimation_Qualifier'], var_name='Date', value_name='Published_Value')

df36['Estimated_Real_Value'] = df36['Published_Value']


# People 25 Years Education
df37 = pd.read_csv('./Updates/STG_FRED_People_25_Years_and_Over_Who_Have_Completed_an_Associates_Degree_or_Higher_5year_estimate_by_County_Percent.txt', sep='\t')
df37['Economic_Measure_Code'] = 'FRED_S1501ACSTOTAL0_00000'
df37['Economic_Measure_Name'] = "People 25 Years and Over Who Have Completed an Associate's Degree or Higher (5-year estimate) by County (Percent)"
...
df37 = df37.rename(columns = {'Region Name':'GeoArea_Name', 'Region Code':'GeoArea_FIPS'})

df37 = df37.melt(id_vars=['GeoArea_FIPS', 'GeoArea_Name', 'Economic_Measure_Code', 'Economic_Measure_Name', 'Published_UOM', 'Unit_of_Measure_Code', 'Calculation_Type', 'Default_Scale', 'Estimation_Qualifier'], var_name='Date', value_name='Published_Value')

df37['Estimated_Real_Value'] = df37['Published_Value']
